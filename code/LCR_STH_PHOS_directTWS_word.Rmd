...
Table1<-lowertoutleprop%>%
  mutate(p_NF_Toutle_Green_only = scales::percent(p_NF_Toutle_Green_only, accuracy=0.1))%>%
  flextable() %>% 
  align(part = "all") %>% # left align
  set_caption(caption = 'Table 3. The proportion of total spawners in two subpopulation areas of the NF Toutle: 1) the Green River and tributaries below the Toutle Sediment Retention Structure (SRS), and 2) Above the SRS' , align_with_table = F) %>% 
...
Table 2 shows the most recent 5 year mean pHOS estimates for all methods for each population and the two subpopulations.
...
Appendix 1: The raw data and annual pHOS estimates for each method for both winter steelhead based on spawning ground surveys, and summer steelhead based on snorkel surveys:
```{r table 3, message=FALSE, warning=FALSE, results="show", echo=FALSE}
Table3
```

Appendix 2: STAN code used to fit the multivariate state space random walk with logit link using a binomial response to the HOS and NOS data from each population to estimate pHOS.
