...
df<-data.frame(reshape2::melt(results$p_all))%>%
  ...
  summarize(draws = list(sample(value, size = 100, replace = T)), .groups = "drop")%>%
  unnest_longer(draws, indices_include = T)%>%
  filter(spawn_year>=2005, !str_detect(population,"summer"))%>%
  spread(key = draws_id, value = draws)

write_rds(df, "draws_v2.rds")
...
# include jacks (<55)?
SQL3 <- paste0("SELECT * FROM tws.mvw_fish WHERE return_yr =",Y,
               " AND species = 'Steelhead' AND run = 'Winter'
               AND survey_location = 'NF Toutle FCF' AND disposition = 'Upstream';")

trapcountstoutle <- select_table_PostgreSQL(database_args=database_args,SQL=SQL3) %>%
  reframe(spawningyear = Y, commonpopname = "NF Toutle (winter)",
          subpop = "NF_Toutle_Total", tsaej = as.numeric(sum(fish_count)))

lowertoutlemissing <- read.csv(paste0("NFToutle_missingdata_",MA,"_BiOp.csv")) %>%
  filter(spawningyear<Y+1) %>%
  rbind(trapcountstoutle) %>%
  group_by(spawningyear) %>%
  mutate(tsaej = ifelse(is.na(tsaej),sum(tsaej,na.rm=T),tsaej)) %>%
  ungroup() %>%
  filter(row_number() <= n()-1)

lowertoutlemissing[lowertoutlemissing==0] <- NA
...
upperkalamamissing <- read.csv("Kalama_missingdata_2017_BiOp.csv") %>%
  filter(spawningyear<Y+1)
...
# upperkalamamissing <- read.csv("Kalama_missingdata_2024_BiOp.csv")
# 
# upperkalamaprop <- upperkalamamissing %>%
#   select(-c(source)) %>%
#   pivot_wider(names_from = subpop,values_from = tsaej) %>%
#   mutate(`Prop Above KFH` = round(as.numeric(Kalama_above_KFH)/as.numeric(Kalama_winter_total), digits=3),
#          spawningyear = as.double(spawningyear)) %>%
...
Table1 <- DF %>%
  ...
  mutate(MAstandard = ifelse(!is.na(MAstandard),paste0("≤", percent(MAstandard)),MAstandard)) %>%
...
flextable(Table1 %>%
  ...
  set_header_labels(MAstandard = "MA pHOS Standard") %>%
  ...
  footnote(i = c(6,17), j = 6, value = as_paragraph(
    c("In the 2017 MA BiOp, pHOS standards for the Kalama are 5% from segregated stocks. For Kalama summer runs, all production is integrated. Methods have not yet been developed to identify what proportion of pHOS for Kalama summer steelhead is from in-basin integrated stocks versus out-of-basin strays, but it is assumed that the vast majority are from in-basin programs. Therefore, the standard of 5% from segregated stocks (which are not produced in-basin) is currently assumed to be met.")),
...
Overall, actions to reduce pHOS (e.g., gene banks, reductions in hatchery plants, changes in release locations of hatchery plants, and changes to recycling protocols) appear to have largely achieved targeted pHOS standards under the `r MA` MA BiOp. While pHOS levels are higher than the `r MA` MA BiOp pHOS standards for Washougal winter steelhead, the segregated winter steelhead program has been eliminated under the new 2024 MA BiOp (NMFS 2024). A new integrated winter steelhead program has been permitted but has not been implemented due to budget constraints.

For Kalama summer steelhead, the five-year mean pHOS was higher than the goal identified in the `r MA` MA BiOp (<5.0% from segregated stocks). However, the pHOS standard in the BiOp specifically identified hatchery spawners from segregated programs. The Skamania stock summer steelhead program in the Kalama River was eliminated with issuance of the 2017 MA BiOp but we still had returning adults from that program through spawn year 2020. As a result, we still saw pHOS levels exceeding 35-40% through spawn year 2019 then saw a substantial drop in the years following spawn year 2019. Methods have not yet been developed to identify what proportion of pHOS for Kalama summer steelhead is from in-basin integrated stocks vs. out of basin strays but it is assumed that the vast majority is from in-basin programs, and therefore the standard of 5% from segregated stocks (which are not produced in-basin) is currently assumed to be met. The Hatchery Scientific Review Group (HSRG) recommended a pHOS standard of less than 30% for primary populations with integrated hatchery programs (Paquet 2011); a standard that is currently being met for this population.

The proportion of hatchery-origin spawners was higher than 5% in one primary Coast stratum population (Mill/Abernathy/Germany/Coal). However, there are no established pHOS standards in the `r MA` MA BiOp for the SW Washington DPS (Grays, Skamokawa/Elochoman, and MAG) as that DPS is not federally listed. The sunsetting of the USFWS conservation hatchery program on MAG is expected to result in lower pHOS relative to the past 5-year average on MAG. 
...
National Marine Fisheries Service (NMFS). 2024. Endangered Species Act Section 7(a)(2) Reinitiated Biological Opinion and Magnuson Stevens Fishery Conservation and Management Act Essential Fish Habitat (EFH) Response. NOAA’s National Marine Fisheries Service’s Continued Implementation of the Mitchell Act Final Environmental Impact Statement Preferred Alternative and Administration of Mitchell Act hatchery funding. December 30, 2024. NMFS Consultation No.: WCRO-2024-03014. 680p.
...
write.csv(AppendixB1, "AppendixB1.csv", row.names = F)
