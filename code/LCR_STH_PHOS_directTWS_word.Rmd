...
SPIdat<-select_table_PostgreSQL(database_args=database_args,SQL="SELECT * FROM spi.vw_ca_nosa_sos2;")%>%
...
AppendixTable1<-lowertoutleprop%>%
  ...
  set_caption(caption = 'Appendix Table 1. The proportion of total spawners in two subpopulation areas of the NF Toutle: 1) the Green River and tributaries below the Toutle Sediment Retention Structure (SRS), and 2) Above the SRS' , align_with_table = F) %>% 
...
Kalama_winter_total<-DF%>%
  filter(population=="KalamabelowKFH winter")%>%
  mutate(across(c("pHOS_gam",
                  "pHOS_gam_lower",
                  "pHOS_gam_upper",
                  "pHOS_ss_rw_lower",
                  ...
                  "pHOS_ss_rw_upper",
                  "pHOS_mom"
                  ), ~ . * 0.1 #avg portion of Kalama Winter runs spawning below KFH (pHOS 0 above for Winters)
  ))%>%
  mutate(HOS=NA,NOS=NA,gear="spawning ground survey and trap",se.fit=NA)%>%
...
## Executive Summary
Hatchery steelhead provide valuable angling opportunities and may be used to supplement natural origin populations while also posing risks to natural origin populations. As a result the proportion of hatchery origin spawners (pHOS) may be monitored as a surrogate for the potential of hatchery origin spawners to negatively affect wild populations. We report estimates of pHOS for Lower Columbia winter and summer steelhead populations based on satistical smoothing models fit to observations of hatchery and natural origin spawners during spawning ground surveys (winter steelhead) and snorkel surveys (summer steelhead). Results currently exclude the Cowlitz and Lewis Basin populations for which estimates are developed as part of other reporting processes associated with dam operator licenses. Estimates for Kalama and NF Toutle/Mainstem Toutle winter run populations are reported both for below-dam subpopulations as well as the whole populations which are generated by developing spawner abundance-weighted averages of pHOS below the dams in these basins based on spawning ground surveys, with above-dam estimates generated with trap counts. Three models are used to estimate pHOS: 1) method of moments where pHOS is calculated annually as (HOS/(HOS + NOS)) for each population in years data are available, 2) a multivariate state-space model fit to the count data using a logit link function and a binomial response using STAN [**rstan**](https://mc-stan.org/users/interfaces/rstan), 3) a Generalized Additive Model fit to the count data using a logit link function and a binomial response where a thin plate spline is fit independently to each population's data using the R package [**mgcv**](https://cran.r-project.org/web/packages/mgcv/mgcv.pdf). The model used to fit the state-space logit-normal random walk is printed below in the appendix. 
...
Figure 1 shows the annual pHOS estimates for each population (or subpopulation for Kalama below KFH and NF Toutle Green only) based on the GAM model with method of moment estimates plotted for comparison.
...
Table 1 shows the most recent 5 year mean pHOS estimates for all methods for each population and the two subpopulations.
...
## Appendix
Appendix 1: weighting subpopulation estimates to obtain total pHOS estimates
...
```{r table 3, message=FALSE, warning=FALSE, results="show", echo=FALSE}
AppendixTable2
```
