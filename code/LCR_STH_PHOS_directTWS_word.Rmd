...
WHERE
  (
    ((tws.header.gear_type_lut_id) = 8)
    AND ((tws.datasheet_trap.species_id) = 3)
    AND ((tws.datasheet_trap.run_lut_id) = 2)
  )
  OR (
    ((tws.header.gear_type_lut_id) = 8)
    AND ((tws.datasheet_trap.species_id) = 3)
...
datxx<-select_table_PostgreSQL(database_args=database_args,SQL=SQLstring)%>%
  ...
  mutate(pHOS_mom = (AD/(AD+UM)))%>%
  arrange(population,spawn_year)

Table2<-datxx%>%
     filter(spawn_year>year(Sys.Date())-5)%>%
     group_by(population)%>%
     summarise(mean_pHOS_mom = scales::percent(mean(pHOS_mom,na.rm=T),accuracy = 0.1)
     )%>%
     flextable() %>% 
     align(part = "all") %>% # left align
     set_caption(caption = 'Table 2. 5-year Mean pHOS by Population for LCR summer steelhead based on snorkel survey counts of clipped and unclipped steelhead. Sample sizes are sufficient so only the method of moments estimates "mean_pHOS_mom" (HOS/(HOS + NOS)) is reported', align_with_table = F) %>% 
     font(fontname = "Calibri (Body)", part = "all") %>% 
     fontsize(size = 6, part = "body") %>% 
     theme_booktabs() %>% # default theme
     rotate(rotation = "tbrl",part='header')%>%
...
Table4<-datxx%>%
  mutate(pHOS_mom=scales::percent(pHOS_mom,accuracy = 0.1))%>%
  flextable() %>% 
  align(part = "all") %>% # left align
  set_caption(caption = 'Table 4. Raw Data and PHOS estimates by year and population for summer steelhead populations based on snorkel survey counts of unclipped and clipped steelhead. Snorkel counts have sufficient sample sizes that statistical estimates have not been needed therefore only method of moment estimates calculated independently each year are provided: (HOS/(HOS + NOS))', align_with_table = F) %>% 
  font(fontname = "Calibri (Body)", part = "all") %>% 
  fontsize(size = 6, part = "body") %>% 
  # add footer if you want
  # add_footer_row(values = "* p < 0.05. ** p < 0.01. *** p < 0.001.", 
...
```{r table 4, message=FALSE, warning=FALSE, results="show", echo=FALSE}
Table4
```
