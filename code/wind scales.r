...
dat<-read_csv(here::here("data/Wind_Steelhead_Scales.csv"))%>%
  filter(run==2)%>%
  mutate(salt_age=gsub("s","S",salt_age))%>%
  ...
  mutate(maturity=ifelse(grepl("S",salt_age),"Repeat","Maiden"))%>%
  group_by(spawn_year,maturity,sy_total_age)%>%
  summarize(n=n())%>%
  pivot_wider(names_from = sy_total_age,values_from = n,values_fill = 0)%>%
  arrange(maturity,spawn_year)%>%
